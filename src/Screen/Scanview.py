...
def on_folder_picked_for_custom_scan(e: ft.FilePickerResultEvent, page: ft.Page,rule,exclusionfiles,VAULT_DIR,quarantinefiles):
    scanned=set()
    if e.path:
        try:
           scan_directory(e.path,scanned)
            ...
            pass
        if scanned:
            Scan(page,scanned,exclusionfiles,rule,False,VAULT_DIR,quarantinefiles)
def ScanView(page: ft.Page,rule,quickfiles,quickpath,deepfiles,exclusionfiles,VAULT_DIR,quarantinefiles):
    file_picker_for_custom_scan = ft.FilePicker(on_result=lambda e: on_folder_picked_for_custom_scan(e, page,rule,exclusionfiles,VAULT_DIR,quarantinefiles))
    ...
    return ft.Container(
        ...
        content=ft.Column(
            [
                ...
                button(page,"Quick Scan","Quickly scans high-risk areas for threats",icon=ft.Icons.SAVED_SEARCH,on_click=lambda _:file_picker_for_quick_scan.get_directory_path() if not quickfiles else Scan(page,quickfiles,exclusionfiles,rule,False,VAULT_DIR,quarantinefiles)),
                button(page,"Deep Scan","A full threat inspection for your entire device",icon=ft.Icons.SCREEN_SEARCH_DESKTOP_SHARP,on_click=lambda _:Scan(page,deepfiles,exclusionfiles,rule,True,VAULT_DIR,quarantinefiles)),
